...
const ACTION_COLS = [
  ...
  },
  {
    ...
    sortable: true,
    render: (r) => (
      <span className="font-medium text-[#041E66]">{r.updated || "—"}</span>
    ),
  },
];
...
const AuditTrailPage = () => {
    ...
    () => [
      {
        ...
        sortable: true,
        render: (r) => (
          <span className="font-semibold text-[#041E66]">{r.user}</span>
        ),
      },
  ...
  return (
    <div className="font-sans">
      ...
      {/* ── View Actions Modal ── */}
      {viewRow && (
        <ViewActionsModal row={viewRow} onClose={() => setViewRow(null)} />
      )}
    </div>
  );
};
