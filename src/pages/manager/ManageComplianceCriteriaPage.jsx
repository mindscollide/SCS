/**
 * pages/manager/ManageComplianceCriteriaPage.jsx
 * ================================================
...
const ManageComplianceCriteriaPage = () => {
  ...
  const [form, setForm]           = useState(() =>
    isEdit
      ? { name: editCriteria.name, desc: editCriteria.desc || '' }
      : EMPTY_FORM
  )
  ...
  const handleAddRatio = () => {
    if (!ratioForm.ratioName) { setRatioErr('Please select a Financial Ratio'); return }
    if (!ratioForm.threshold) { setRatioErr('Please enter a Threshold value'); return }
    const val = parseFloat(ratioForm.threshold)
    if (isNaN(val)) { setRatioErr('Invalid threshold value'); return }
    if (ratioForm.unit === '%' && (val < 0.1 || val > 100))   { setRatioErr('Value must be 0.1–100'); return }
    if (ratioForm.unit === '#' && (val < 0.1 || val > 1000))  { setRatioErr('Value must be 0.1–1000'); return }

    setAddedRatios(prev => [...prev, {
      id:        Date.now(),
  ...
  const tableColumns = [
    ...
    { key: 'threshold', title: 'Threshold', render: r => <span className="text-[#041E66]">{r.threshold}</span> },
    {
      key: 'type',
      title: 'Type',
      render: r => (
    ...
    <div className="font-sans">
      ...
      <div className="bg-[#EFF3FF] rounded-xl px-4 py-3 mb-3 border border-slate-200">
        ...
        <div className="flex justify-center gap-4 mb-10">
          ...
          <StepTab num={2} sublabel="Add Financial Ratios" active={step === 2} onClick={goToStep2} />
        </div>
        ...
        {step === 2 && (
          <div className="max-w-5xl mx-auto space-y-5">
            ...
            {ratioErr && (
              <p className="text-[12px] text-red-500 font-medium">{ratioErr}</p>
            )}
