/**
 * pages/manager/BulkActionPage.jsx
 * ===================================
...
const MOCK_DATA = [
  { id: 1, quarter: 'December - 2025',  ticker: 'BGL',    company: 'Balochistan Glass Ltd',          sector: 'GLASS & CERAMICS', sentBy: 'Humaid Afzal',     sentOn: '2025-12-21' },
  { id: 2, quarter: 'September - 2025', ticker: 'MUGHAL', company: 'Mughal Iron & Steel Industries', sector: 'Iron & Steel',     sentBy: 'Huzeifa Jahangir', sentOn: '2025-09-05' },
  { id: 3, quarter: 'June - 2025',      ticker: 'PIOC',   company: 'Paracha Iron Limited',           sector: 'Steel Industry',   sentBy: 'Muhammad Aamir',   sentOn: '2025-06-25' },
  { id: 4, quarter: 'March - 2025',     ticker: 'POC',    company: 'Pioneer Cement Limited',         sector: 'Cement',           sentBy: 'Jawad Faisal',     sentOn: '2025-03-10' },
  { id: 5, quarter: 'December - 2024',  ticker: 'BGL',    company: 'Balochistan Glass Ltd',          sector: 'GLASS & CERAMICS', sentBy: 'Muhammad Hassan',  sentOn: '2024-12-25' },
  { id: 6, quarter: 'September - 2024', ticker: 'LUCK',   company: 'Lucky Cement',                  sector: 'Cement',           sentBy: 'Bilal Khan',       sentOn: '2024-09-14' },
]
...
const CHIP_LABELS = {
  ...
  quarter: 'Quarter', sentBy: 'Sent By', sentFrom: 'From', sentTo: 'To',
}
...
const BulkActionPage = () => {
    ...
    setRows(
      sourceData.current.filter(r => {
        ...
        const textMatch = Object.entries(f).every(([k, v]) => {
          ...
          if (k === 'company') {
            return r.company?.toLowerCase().includes(v.toLowerCase()) ||
        ...
        const dateMatch = (!from || (sent && sent >= from)) &&
                          (!to   || (sent && sent <= to))
  ...
  const handleSearch = useCallback(() => {
    const next = {}
    Object.entries(filters).forEach(([k, v]) => { if (v?.trim?.() ?? v) next[k] = v })
    setApplied(next)
    fetchData(next)
  ...
  const SortIcon = ({ col }) => (
    <span className="inline-flex flex-col ml-1 leading-none select-none">
      <span className={`text-[9px] ${sortCol === col && sortDir === 'asc'  ? 'text-[#041E66]' : 'text-[#a0aec0]'}`}>▲</span>
      <span className={`text-[9px] ${sortCol === col && sortDir === 'desc' ? 'text-[#041E66]' : 'text-[#a0aec0]'}`}>▼</span>
    </span>
  )

  // ── Approve / Decline submit ──────────────────────────────────────────────
  const handleAction = useCallback((notes) => {
    ...
    <div className="font-sans">
              ...
              <tbody>
                {displayRows.length === 0 ? (
                  ...
                  <tr
                    ...
                    onMouseLeave={e => {
                      e.currentTarget.style.backgroundColor = selected.has(row.id) ? '#e8faf4' : '#ffffff'
                    }}
                  >
                    {/* Checkbox */}
                    ...
                    {/* Ticker */}
                    <td className="px-4 py-3 font-mono font-bold text-[#0B39B5]">
                      {row.ticker}
                    </td>
      ...
      {modal && (
        <RequestActionModal
          ...
          infoFields={[
            { label: 'Selected Records', value: `${selected.size} record${selected.size !== 1 ? 's' : ''}` },
